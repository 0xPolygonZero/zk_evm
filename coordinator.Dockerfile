...
RUN sed -i 's#\[workspace\]#\[workspace\]\nmembers = \["zero_bin\/leader", "zero_bin\/coordinator", "zero_bin\/prover", "zero_bin\/rpc", "zero_bin\/common", \
...
COPY zero_bin/coordinator zero_bin/coordinator
...
RUN \
  ...
  touch zero_bin/coordinator/src/main.rs && \
...
RUN mkdir -p zero_bin/coordinator
