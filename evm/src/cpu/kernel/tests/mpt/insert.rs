...
fn test_state_trie(mut state_smt: Smt<MemoryDb>, k: Key, value: U256) -> Result<()> {
    let trie_inputs = TrieInputs {
        state_smt: state_smt.serialize(),
        transactions_trie: Default::default(),
        receipts_trie: Default::default(),
    ...
    interpreter
        ...
        .expect("The stack should not overflow");
    interpreter
        .push(state_trie_ptr) // Initial length of the trie data segment, unused.
        .expect("The stack should not overflow");
    interpreter.run()?;

    let state_smt_hash = interpreter.pop().expect("The stack should not be empty");
    state_smt.set(k, value);
