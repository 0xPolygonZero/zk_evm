...
global encode_or_hash_node_new:
    ...
    // stack: hash_ptr, encode_value, cur_len, next_addr_ptr, next_slot_ptr, next_hash_node_ptr, retdest
    %mload_trie_data
    // stack: hash, encode_value, cur_len, next_addr_ptr, next_slot_ptr, next_hash_node_ptr, retdest
    // Update the length of the `TrieData` segment: there are only two 
    // elements in a hash node.
    SWAP2 %add_const(2)
    %stack
        (cur_len, encode_value, hash, next_addr_ptr, next_slot_ptr, next_hash_node_ptr, retdest) -> 
...
encode_node_extension_after_encode_child:
    ...
    %alloc_rlp_block
    // stack: rlp_start, result, result_len, node_payload_ptr, cur_len, next_addr_ptr, next_slot_ptr, next_hash_node_ptr, next_addr_ptr, next_slot_ptr, next_hash_node_ptr, retdest
