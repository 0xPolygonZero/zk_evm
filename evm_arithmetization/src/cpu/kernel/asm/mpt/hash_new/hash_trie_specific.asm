...
global mpt_hash_storage_trie_new:
    // stack: node_ptr, cur_len, next_slot_ptr, next_hash_node_ptr, retdest
    %stack (node_ptr, cur_len) -> (node_ptr, encode_storage_value_new, cur_len)
    %jump(mpt_hash_storage)

%macro mpt_hash_storage_trie_new
    %stack (node_ptr, cur_len, next_slot_ptr, next_hash_node_ptr) -> (node_ptr, cur_len, next_slot_ptr, next_hash_node_ptr, %%after)
...
global encode_account_new:
    ...
    ADD ADD
    // stack: payload_len, rlp_addr, value_ptr, cur_len, next_slot_ptr, next_hash_node_ptr, retdest
    SWAP1
    // stack: rlp_addr, payload_len, value_ptr, cur_len, next_slot_ptr, next_hash_node_ptr, retdest
    DUP2 %rlp_list_len
    // stack: list_len, rlp_addr, payload_len, value_ptr, cur_len, next_slot_ptr, next_hash_node_ptr, retdest
    SWAP1
    // stack: rlp_addr, list_len, payload_len, value_ptr, cur_len, next_slot_ptr, next_hash_node_ptr, retdest
    %encode_rlp_multi_byte_string_prefix
    // stack: rlp_pos_2, payload_len, value_ptr, cur_len, next_slot_ptr, next_hash_node_ptr, retdest
    %encode_rlp_list_prefix
    // stack: rlp_pos_3, value_ptr, cur_len, next_slot_ptr, next_hash_node_ptr, retdest
    DUP2 %mload_trie_data // nonce = value[0]
    // stack: nonce, rlp_pos_3, value_ptr, cur_len, next_slot_ptr, next_hash_node_ptr, retdest
    SWAP1 %encode_rlp_scalar
    // stack: rlp_pos_4, value_ptr, cur_len, next_slot_ptr, next_hash_node_ptr, retdest
    DUP2 %increment %mload_trie_data // balance = value[1]
    // stack: balance, rlp_pos_4, value_ptr, cur_len, next_slot_ptr, next_hash_node_ptr, retdest
    SWAP1 %encode_rlp_scalar
    // stack: rlp_pos_5, value_ptr, cur_len, next_slot_ptr, next_hash_node_ptr, retdest
    DUP3
    DUP3 %add_const(2) %mload_trie_data // storage_root_ptr = value[2]
    // stack: storage_root_ptr, cur_len, rlp_pos_5, value_ptr, cur_len, next_slot_ptr, next_hash_node_ptr, retdest
    %stack
        (storage_root_ptr, cur_len, rlp_pos_5, value_ptr, cur_len, next_slot_ptr, next_hash_node_ptr) ->
    ...
    %stack
        (storage_root_digest, new_len, next_slot_ptr, next_hash_node_ptr, rlp_pos_five, value_ptr, cur_len) -> 
        (rlp_pos_five, storage_root_digest, value_ptr, new_len, next_slot_ptr, next_hash_node_ptr)
    %encode_rlp_256
    ...
    %stack(rlp_pos_7, new_len, next_slot_ptr, next_hash_node_ptr, retdest) -> (retdest, rlp_pos_7, new_len, next_slot_ptr, next_hash_node_ptr)
    JUMP
