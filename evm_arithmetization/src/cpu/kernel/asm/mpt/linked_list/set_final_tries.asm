// Delete all leaves from the trie whose root is `node_ptr` and
// return 
...
global insert_all_accounts:
    ...
    %stack
        ...
        (storage_ptr_ptr, storage_root_ptr, key, after_insert_all_slots, storage_root_ptr_ptr, key)
    %jump(insert_all_slots)
afert_insert_all_slots:
...
after_insert_account:
    // stack: root_ptr', account_ptr_ptr, storage_ptr_ptr'
    SWAP1
    %next_account
    // stack: account_ptr_ptr', root_ptr', storage_ptr_ptr'
    %jump(insert_all_accounts)

no_more_accounts:
    // stack: key, storage_ptr_ptr, root_ptr, account_ptr_ptr, retdest
    %stack (key, storage_ptr_ptr, root_ptr, account_ptr_ptr, retdest) ->(retdets, root_ptr)
    JUMP
...
// Delete all the accounts which where deleted from the intial state.
// Delete also all slots deleted from the storage trie of non-deleted accounts.
// Pre stack: account_ptr_ptr, root_ptr, storage_ptr_ptr, retdest
// Post stack: new_root_ptr.
global delete_removed_accounts:
    DUP1
    %mload_global_metadata(@GLOBAL_METADATA_INITAL_ACCOUNTS_LINKED_LIST_LEN)
    EQ
    ...
    %add_const(2) // storage_root_ptr = payload_ptr[2]
    DUP1
    %mload_trie_data
    %stack
