...
pub(crate) fn evm_constants() -> HashMap<String, U256> {
    ...
    c.insert(
        ADDRESS_GLOBAL_EXIT_ROOT_MANAGER_L2.0.into(),
        U256::from_big_endian(&ADDRESS_GLOBAL_EXIT_ROOT_MANAGER_L2.1),
    );
    c.insert(
        GLOBAL_EXIT_ROOT_STORAGE_POS.0.into(),
        U256::from(GLOBAL_EXIT_ROOT_STORAGE_POS.1),
    );
