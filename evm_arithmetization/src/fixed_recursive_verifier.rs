...
/// [`AllVerifierData`] is much lighter, allowing anyone to verify block proofs,
/// wrapped block proofs and independently aggregated block proofs regardless of
...
where
    ...
    C::Hasher: AlgebraicHasher<F>,
{
    /// Verifies a chain's block proof generated with the
    /// [`AllRecursiveCircuits`] counterpart of this [`AllVerifierData`].
    ///
    /// Block proofs can represent an arbitrarily long sequence of contiguous
    /// blocks from a known checkpoint block height.
    ...
    /// Verifies a chain's wrapped block proof generated with the
    /// [`AllRecursiveCircuits`] counterpart of this [`AllVerifierData`].
    ///
    /// Wrapped block proofs are the final version of block proofs, in that they
    /// are more succinct and only exposes minimal public inputs related to
    /// the chain and its latest state.
    ...
    /// Verifies an aggregated block proof generated with the
    /// [`AllRecursiveCircuits`] counterpart of this [`AllVerifierData`].
    ///
    /// Aggregated block proofs consist in an aggregation of arbitrarily many,
    /// independent wrapped block proofs generated by arbitrarily many distinct
    /// chains.
