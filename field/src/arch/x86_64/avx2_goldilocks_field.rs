...
//    res_hi = a_hi + b_hi - carry_mask Notice that carry_mask is subtracted,
//    not added. This is because AVX comparison instructions return -1 (all bits
...
//    enable this trick. Example: addition with carry. i. a_lo_s = shift(a_lo)
//    ii. res_lo_s = a_lo_s + b_lo iii. carry_mask = res_lo_s <s a_lo_s iv.
//    res_lo = shift(res_lo_s) v. res_hi = a_hi + b_hi - carry_mask The suffix
//    _s denotes a value that has been shifted by 1 << 63. The result of
//    addition is shifted if exactly one of the operands is shifted, as is the
//    case on line ii. Line iii. performs a signed comparison res_lo_s <s a_lo_s
//    on shifted values to emulate unsigned comparison res_lo <u a_lo on
...
//    (128-bit) numbers we can do: i. a_lo_s = shift(a_lo) ii. tmp_lo_s = a_lo_s
//    + b_lo iii. tmp_carry_mask = tmp_lo_s <s a_lo_s iv. tmp_hi = a_hi + b_hi -
//    tmp_carry_mask v. res_lo_s = tmp_lo_s + c_lo vi. res_carry_mask = res_lo_s
//    <s tmp_lo_s vii. res_lo = shift(res_lo_s) viii. res_hi = tmp_hi + c_hi -
//    res_carry_mask Notice that the above 3-value addition still only requires
//    two calls to shift, just like our 2-value addition.
