...
impl<F: Extendable<4>> Field for QuarticExtension<F> {
    ...
    // `p^4 - 1 = (p - 1)(p + 1)(p^2 + 1)`. The `p - 1` term has a two-adicity of
    // `F::TWO_ADICITY`. As long as `F::TWO_ADICITY >= 2`, `p` can be written as
    // `4n + 1`, so `p + 1` can be written as `2(2n + 1)`, which has a 2-adicity
    // of 1. A similar argument can show that `p^2 + 1` also has a 2-adicity of
    // 1.
