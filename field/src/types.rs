    ...
    + DeserializeOwned
{
    ...
    fn batch_multiplicative_inverse(x: &[Self]) -> Vec<Self> {
        // This is Montgomery's trick. At a high level, we invert the product of the
        // given field elements, then derive the individual inverses from that
        // via multiplication.

        // The usual Montgomery trick involves calculating an array of cumulative
        // products, resulting in a long dependency chain. To increase
        // instruction-level parallelism, we compute WIDTH separate cumulative
        // product arrays that only meet at the end.

        // Higher WIDTH increases instruction-level parallelism, but too high a value
        // will cause us to run out of registers.
        const WIDTH: usize = 4;
        // JN note: WIDTH is 4. The code is specialized to this value and will need
        // modification if it is changed. I tried to make it more generic, but Rust's
        // const generics are not yet good enough.
        ...
        // Fill buf with cumulative product of x, only taking every 4th value.
        // Concretely, buf will be [
        //   x[0], x[1], x[2], x[3],
        //   x[0] * x[4], x[1] * x[5], x[2] * x[6], x[3] * x[7],
        //   x[0] * x[4] * x[8], x[1] * x[5] * x[9], x[2] * x[6] * x[10], x[3] * x[7] *
        // x[11],   ...
        // ].
        // If n is not a multiple of WIDTH, the result is truncated from the end. For
        // example, for n == 5, we get [x[0], x[1], x[2], x[3], x[0] * x[4]].
        let mut buf: Vec<Self> = Vec::with_capacity(n);
        // cumul_prod holds the last WIDTH elements of buf. This is redundant, but it's
        // how we convince LLVM to keep the values in the registers.
    ...
    /// Computes the subgroup generated by the root of unity of a given order
    /// generated by `Self::primitive_root_of_unity`.
    ...
    /// Computes a coset of a multiplicative subgroup whose order is known in
    /// advance.
    ...
    /// Returns `n % Self::characteristic()`. May be cheaper than
    /// from_noncanonical_u128 when we know that `n < 2 ** 96`.
    #[inline]
    ...
    fn kth_root_u64(&self, k: u64) -> Self {
        ...
        // By Fermat's little theorem, x^p = x and x^(p - 1) = 1, so x^(p + n(p - 1)) =
        // x for any n. Our assumption that the k'th root operation is a
        // permutation implies gcd(p - 1, k) = 1, so there exists some n such
        // that p + n(p - 1) is a multiple of k. Once we find such an n,
...
pub trait Field64: Field {
    ...
    /// Returns `n` as an element of this field. Assumes that `0 <= n <
    /// Self::ORDER`.
    // TODO: Move to `Field`.
    // TODO: Should probably be unsafe.
    #[inline]
