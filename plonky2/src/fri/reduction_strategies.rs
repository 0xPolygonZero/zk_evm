...
pub enum FriReductionStrategy {
    ...
    /// `ConstantArityBits(arity_bits, final_poly_bits)` applies reductions of
    /// arity `2^arity_bits` until the polynomial degree is less than or
    /// equal to `2^final_poly_bits` or until any further
    /// `arity_bits`-reduction makes the last FRI tree have height less than
    /// `cap_height`. This tends to work well in the recursive setting, as
    /// it avoids needing multiple configurations of gates used in FRI
    /// verification, such as `InterpolationGate`.
    ConstantArityBits(usize, usize),

    /// `MinSize(opt_max_arity_bits)` searches for an optimal sequence of
    /// reduction arities, with an optional max `arity_bits`. If this proof
    /// will have recursive proofs on top of it, a max `arity_bits` of 3 is
    /// recommended.
...
fn min_size_arity_bits(
    ...
    // 2^4 is the largest arity we see in optimal reduction sequences in practice.
    // For 2^5 to occur in an optimal sequence, we would need a really massive
    // polynomial.
...
fn min_size_arity_bits_helper(
    ...
    // The largest next_arity_bits to search. Note that any optimal arity sequence
    // will be monotonically non-increasing, as a larger arity will shrink more
    // Merkle proofs if it occurs earlier in the sequence.
...
/// Compute the approximate size of a FRI proof with the given reduction
/// arities. Note that this ignores initial evaluations, which aren't affected
/// by arities, and some other minor contributions. The result is measured in
/// field elements.
