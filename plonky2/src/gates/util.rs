...
/// Permits us to abstract the underlying memory layout. In particular, we can
/// make a matrix of constraints where every column is an evaluation point and
/// every row is a constraint index, with the matrix stored in row-contiguous
/// form.
...
impl<'a, P: PackedField> StridedConstraintConsumer<'a, P> {
    pub fn new(buffer: &'a mut [P::Scalar], stride: usize, offset: usize) -> Self {
        ...
        // `wrapping_add` is needed to avoid undefined behavior. Plain `add` causes UB
        // if 'the ... resulting pointer [is neither] in bounds or one byte past
        // the end of the same allocated object'; the UB results even if the
        // pointer is not dereferenced. `end` will be more than one byte past
        // the buffer unless `offset` is 0. The same applies to `start` if the buffer
        // has length 0 and the offset is not 0.
        // We _could_ do pointer arithmetic without `wrapping_add`, but the logic would
    ...
    pub fn one(&mut self, constraint: P) {
        if self.start != self.end {
            ...
            // exhausted our buffer (and hence we're setting `self.start` to point past the
            // end).
