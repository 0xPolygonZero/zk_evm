...
function statistics() {
  ...
  printf "\n\nStatistics\n" | tee -a $RESULTS
  echo "---------------------------------------------------------------------------------------"    | tee -a $RESULTS
  printf "%10s Total blocks\n" $BLOCKS_TESTED | tee -a $RESULTS
  printf "%10s Total transactions\n" $SUMTOTAL | tee -a $RESULTS
  printf "%10s Transactions with prefetched JDT\n" $SUMOK | tee -a $RESULTS
  printf "%10s Transactions without prefetched JDT\n" $SUMFAIL | tee -a $RESULTS
  printf "%10s Prefetch JDT failure rate\n" $([[ $SUMTOTAL -eq 0 ]] && echo "0" || echo "$(($SUMFAIL * 100 / $SUMTOTAL))%") | tee -a $RESULTS
  printf "%10s Prefetch JDT success rate\n" $([[ $SUMTOTAL -eq 0 ]] && echo "0" || echo "$(($SUMOK * 100 / $SUMTOTAL))%") | tee -a $RESULTS
  prinft "Error types encountered (see legend above)\n" | tee -a $RESULTS
  printf "%10s Zeroes\n" $ZEROES | tee -a $RESULTS
  printf "%10s Ones\n" $ONES | tee -a $RESULTS
  printf "%10s Twos\n" $TWOS | tee -a $RESULTS
  printf "%10s Threes\n" $THREES | tee -a $RESULTS
  printf "%10s Fours\n" $FOURS | tee -a $RESULTS
  printf "%10s Fives\n" $FIVES | tee -a $RESULTS
  printf "%10s Sixes\n" $SIXES | tee -a $RESULTS
  printf "%10s Sevens\n" $SEVENS | tee -a $RESULTS
  printf "%10s Eights\n" $EIGHTS | tee -a $RESULTS
  printf "%10s Nines\n" $NINES | tee -a $RESULTS
  printf "%10s Timeouts\n" $TIMEOUTS | tee -a $RESULTS
...
for BLOCK in $BLOCKS; do
  ...
  DURATION_RPC=`date -u -d @"$SECONDS" +'%0Hh%0Mm%0Ss'`
  ...
  export RUST_LOG=info
  SECONDS=0
  timeout 20h nice -19 -- ./prove_stdio.sh $WITNESS test_only $BLOCK
  EXITCODE=$?
  TOTALTIME=`echo -n $(($TOTALTIME + $SECONDS))`
  DURATION_PRV=`date -u -d @"$SECONDS" +'%0Hh%0Mm%0Ss'`
  TOTALTIME=`date -u -d @"$TOTALTIME" +'%0Hh%0Mm%0Ss'`
  if [ $EXITCODE -eq 0 ]
  then
    VERDICT="success"
  else
