...
// `debug`)...
// pub fn parse_just_to_instructions(bytes: Vec<u8>) ->
// CompactParsingResult<Vec<Instruction>> {     let witness_bytes =
// WitnessBytes::<DebugCompactCursor>::new(bytes);     let (_, entries) =
// witness_bytes.process_into_instructions_and_header()?;
...
//             _ => unreachable!(
//                 "Found a non-instruction at a stage when we should only have
// instructions!"             ),
//         })
//         .collect())
// }
