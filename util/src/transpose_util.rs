...
/// `M[i, j] == arr[(i + x << lb_stride) + j + x]` for `0 <= i, j < 1 <<
/// lb_size`. The transposition swaps `M[i, j]` and `M[j, i]`.
///
/// SAFETY:
/// Make sure that `(i + x << lb_stride) + j + x` is a valid index in `arr` for
/// all `0 <= i, j < 1 << lb_size`. Ensure also that `lb_size <= lb_stride` to
/// prevent overlap.
...
/// `M0[i, j] == arr[(i + x << lb_stride) + j + y]`, `M1[i, j] == arr[i + x + (j
/// + y << lb_stride)]` for `0 <= i, j < 1 << lb_size. The transposition swaps
/// `M0[i, j]` and `M1[j, i]`.
///
/// SAFETY:
/// Make sure that `(i + x << lb_stride) + j + y` and `i + x + (j + y <<
/// lb_stride)` are valid indices in `arr` for all `0 <= i, j < 1 << lb_size`.
/// Ensure also that `lb_size <= lb_stride` to prevent overlap.
