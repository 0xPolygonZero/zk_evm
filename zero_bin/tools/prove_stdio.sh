...
if [[ $TEST_ONLY == "test_only" ]]; then
  cargo run --quiet --release --bin leader -- --test-only --runtime in-memory --load-strategy on-demand --batch-size $BATCH_SIZE --proof-output-dir $PROOF_OUTPUT_DIR stdio < $INPUT_FILE |& tee $TEST_OUT_PATH
