...
if [[ $TEST_ONLY == "test_only" ]]; then
    cargo run --quiet --release --package zero_bin_common --bin leader -- --test-only --runtime in-memory --load-strategy on-demand --block-batch-size $BLOCK_BATCH_SIZE --proof-output-dir $PROOF_OUTPUT_DIR stdio < $INPUT_FILE &> $TEST_OUT_PATH
